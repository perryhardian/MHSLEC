...
let AnalyticsService = class AnalyticsService {
    ...
    async getDashboard(userId, query) {
        ...
        const [user, income, expense, recentTransactions, budgets, goals] = await Promise.all([
        ...
        const budgetSummaries = await Promise.all(budgets.map((budget) => this.getBudgetSummary(budget)));
        const expenseByCategory = await this.getExpenseByCategory(userId, startDate, endDate);
        ...
        const financialHealthScore = this.calculateFinancialHealthScore({
            ...
            expectedDailySpend: user?.expectedDailySpend === null || user?.expectedDailySpend === undefined
                ? null
    ...
    getSavingsSummary(goals) {
        const totalTargetAmount = goals.reduce((total, goal) => total + Number(goal.targetAmount), 0);
        const totalCurrentAmount = goals.reduce((total, goal) => total + Number(goal.currentAmount), 0);
        const averageProgressPercentage = totalTargetAmount > 0
    ...
    calculateFinancialHealthScore(input) {
        const availableDailyMoney = input.remainingDays > 0 ? input.currentMoney / input.remainingDays : 0;
        const dailyShortage = input.expectedDailySpend === null
            ? 0
            : Math.max(input.expectedDailySpend - availableDailyMoney, 0);
        const dailyShortageRate = input.expectedDailySpend === null || availableDailyMoney <= 0
            ? input.expectedDailySpend === null
                ? 0
                : 1
            : dailyShortage / availableDailyMoney;
        const dailyCapacityScore = input.expectedDailySpend === null
            ? 0
            : Math.max(0, Math.min(70 * (1 - dailyShortageRate), 70));
        const budgetScore = input.budgetSummaries.length === 0
            ? 30
            : Math.max(0, 30 -
    ...
    upsertFinancialHealthSnapshot(input) {
    ...
    getRemainingDaysInMonth(referenceDate, month, year) {
